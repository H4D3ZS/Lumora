...
class RouteParser {
     ...
     * ```
     */
    static matchPath(routePath, urlPath) {
     ...
     * ```
     */
    static findMatchingRoute(routes, path) {
        const matches = [];
     ...
     * @returns Match score
     */
    static calculateMatchScore(routePath, urlPath) {
        let score = 0;
     ...
     * ```
     */
    static buildPath(route, params = {}, query = {}) {
        let path = route.path;
     ...
     * ```
     */
    static validateParameters(params, definitions) {
        const errors = [];
        for (const def of definitions) {
            ...
            if (def.pattern && typeof value === 'string') {
                ...
                if (!regex.test(value)) {
                    errors.push(`Parameter '${def.name}' does not match pattern: ${def.pattern}`);
                }
            }
        }
        return {

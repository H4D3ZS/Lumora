...
exports.getReactParser = getReactParser;
exports.resetReactParser = resetReactParser;
...
class ReactParser {
     ...
     */
    serializeTSTypeParameterDeclaration(node) {
     ...
     */
    serializeTSExpressionWithTypeArguments(node) {
     ...
     */
    serializeTSTypeParameterInstantiation(node) {
        ...
        (0, traverse_1.default)(ast, {
            ...
            VariableDeclarator: (path) => {
                if (t.isIdentifier(path.node.id) &&
                    ...
                    this.isArrowFunctionComponent(path.get('init'))) {
                    const arrowFunc = path.node.init;
                    components.push({
                        name: path.node.id.name,
                        type: 'function',
                        node: arrowFunc,
                        props: arrowFunc.params[0],
    ...
    convertComponent(component) {
        const node = (0, ir_utils_1.createNode)(component.name, {}, [], component.loc?.start.line || 0);
        // Extract props
        if (component.props) {
        ...
        if (genericParams.length > 0) {
            node.metadata.genericParameters = genericParams;
        }
        ...
        if (component.type === 'class' && t.isClassDeclaration(component.node)) {
            ...
            if (decorators.length > 0) {
                node.metadata.decorators = decorators;
            }
        }
     ...
     */
    determineLifecycleType(dependencies) {
        if (!dependencies) {
     ...
     */
    extractEffectHandler(effectFunction) {
        if (!effectFunction) {
    ...
    findHooks(component) {
        ...
        (0, traverse_1.default)(t.file(t.program([t.isStatement(bodyNode) ? bodyNode : t.expressionStatement(bodyNode)])), {
            CallExpression: (path) => {
     ...
     */
    extractUseStateInfo(path, hookInfo) {
     ...
     */
    extractUseEffectInfo(path, hookInfo) {
     ...
     */
    extractUseContextInfo(path, hookInfo) {
     ...
     */
    extractUseRefInfo(path, hookInfo) {
     ...
     */
    extractUseMemoInfo(path, hookInfo) {
     ...
     */
    extractUseCallbackInfo(path, hookInfo) {
     ...
     */
    extractCustomHookInfo(path, hookInfo) {
     ...
     */
    extractHelperFunctions(component) {
        const helpers = [];
        const bodyNode = component.node;
    ...
    convertJSXElement(jsx) {
        ...
        return (0, ir_utils_1.createNode)(tagName, props, children, jsx.loc?.start.line || 0);
    }
    /**
    ...
    convertJSXChildren(children) {
        ...
        children.forEach(child => {
            if (t.isJSXElement(child)) {
                ...
                if (text) {
                    nodes.push((0, ir_utils_1.createNode)('Text', { text }, [], child.loc?.start.line || 0));
                }
            }
     ...
     */
    extractHandlerParams(expression) {
        if (t.isJSXEmptyExpression(expression))
            return [];
    ...
    serializeExpression(expr) {
        ...
        if (t.isObjectExpression(expr)) {
            const properties = expr.properties.map(prop => {
                if (t.isObjectProperty(prop)) {
                    ...
                    const value = this.serializeExpression(prop.value);
                    return `${key}: ${value}`;
                }
        ...
        if (t.isTemplateLiteral(expr)) {
            ...
            const expressions = expr.expressions.map(e => this.serializeExpression(e));
            let result = '`';
        ...
        if (t.isNewExpression(expr)) {
            // new Constructor()
            const callee = this.serializeExpression(expr.callee);
     ...
     */
    serializeMemberExpression(expr) {
        const object = this.serializeExpression(expr.object);
        if (expr.computed) {
            // obj[prop]
     ...
     */
    serializeObjectPattern(pattern) {
     ...
     */
    serializeTSType(tsType) {
        // Primitive types
        if (t.isTSStringKeyword(tsType))
            return 'string';
        if (t.isTSNumberKeyword(tsType))
            return 'number';
        if (t.isTSBooleanKeyword(tsType))
            return 'boolean';
        if (t.isTSAnyKeyword(tsType))
            return 'any';
        if (t.isTSVoidKeyword(tsType))
            return 'void';
        if (t.isTSNullKeyword(tsType))
            return 'null';
        if (t.isTSUndefinedKeyword(tsType))
            return 'undefined';
        if (t.isTSNeverKeyword(tsType))
            return 'never';
        if (t.isTSUnknownKeyword(tsType))
            return 'unknown';
        if (t.isTSObjectKeyword(tsType))
            return 'object';
        if (t.isTSSymbolKeyword(tsType))
            return 'symbol';
        if (t.isTSBigIntKeyword(tsType))
            return 'bigint';
        // Array types
        ...
        if (t.isTSTupleType(tsType)) {
            const elements = tsType.elementTypes.map(el => {
                ...
                return this.serializeTSType(el);
            }).join(', ');
            return `[${elements}]`;
        }
     ...
     */
    serializeTSIndexSignature(node) {
        const readonly = node.readonly ? 'readonly ' : '';
        const params = node.parameters.map(param => {
            if (t.isIdentifier(param)) {
                const typeAnnotation = param.typeAnnotation && t.isTSTypeAnnotation(param.typeAnnotation)
    ...
    extractValue(expression) {
        ...
        if (t.isSpreadElement(expression))
            return null;
        if (t.isArgumentPlaceholder(expression))
            return null;
        ...
        if (t.isArrayExpression(expression)) {
            return expression.elements.map(el => this.extractValue(el));
        }
...
function getReactParser(config) {
    if (!parserInstance) {

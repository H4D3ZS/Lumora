...
class StateBridge {
     ...
     */
    convertReactToFlutter(state, componentName) {
        try {
            if (state.type === 'local') {
     ...
     */
    convertLocalStateToFlutter(state, componentName) {
...
${setters}
  ...
  Widget build(BuildContext context) {
     ...
     */
    convertGlobalStateToFlutter(state, componentName) {
     ...
     */
    generateBlocState(state, componentName) {
...
${blocClass}`;
     ...
     */
    generateBlocEvent(variable, eventName) {
...
class ${eventClassName} extends ${eventName} {
  ...
  List<Object?> get props => [${variable.name}];
     ...
     */
    generateBlocEventHandler(variable, eventName, stateName) {
        const eventClassName = `Update${this.capitalize(variable.name)}Event`;
     ...
     */
    generateRiverpodState(state, componentName) {
...
${provider}`;
     ...
     */
    generateRiverpodMethod(variable, stateClassName) {
     ...
     */
    generateProviderState(state, componentName) {
...
${setters}
     ...
     */
    generateProviderSetter(variable) {
        const type = this.mapTypeToFlutter(variable.type);
     ...
     */
    generateGetXState(state, componentName) {
...
${methods}
     ...
     */
    formatInitialValueForFlutter(value, type) {
        if (value === undefined || value === null) {
     ...
     */
    convertUseStateToFlutter(hookInfo, componentName) {
  ...
  void ${hookInfo.setterName || `set${this.capitalize(hookInfo.stateName)}`}(${type} value) {
     ...
     */
    convertUseContextToInheritedWidget(hookInfo, componentName) {
...
// final ${hookInfo.stateName} = ${widgetName}.of(context)?.value;`;
     ...
     */
    convertUseReducerToBloc(hookInfo, componentName) {
        const blocName = `${componentName}Bloc`;
        const eventName = `${componentName}Event`;
        const stateName = `${componentName}State`;
...
${blocClass}`;
     ...
     */
    generateBlocEventFromAction(action, eventName) {
...
class ${eventClassName} extends ${eventName} {
  const ${eventClassName}();
     ...
     */
    generateBlocEventHandlerFromAction(action, eventName, stateName) {
        const eventClassName = `${this.capitalize(action.type)}Event`;
     ...
     */
    convertFlutterToReact(flutterState, componentName) {
        try {
     ...
     */
    convertStatefulWidgetToReact(flutterState, componentName) {
...
function ${componentName}() {
  ...
  return (
     ...
     */
    convertInheritedWidgetToContext(flutterState, componentName) {
...
export function use${componentName}() {
  ...
  return context;
     ...
     */
    convertBlocToUseReducer(flutterState, componentName) {
...
${hookUsage}`;
     ...
     */
    convertRiverpodToReact(flutterState, componentName) {
...
${updateMethods}
     ...
     */
    convertProviderToReact(flutterState, componentName) {
     ...
     */
    formatFlutterValueForReact(value, type) {
        if (!value) {
            // Return default values based on type
            if (type.includes('String'))
                return "''";
            if (type.includes('int') || type.includes('double') || type.includes('num'))
                return '0';
     ...
     */
    preserveState(oldState, newState) {
     ...
     */
    migrateState(oldState, oldDefinition, newDefinition) {
        const migratedState = {};
    ...
    canConvertType(fromType, toType) {
        ...
        if (fromType === 'any' || fromType === 'dynamic')
            return true;
        if (toType === 'any' || toType === 'dynamic')
            return true;
        return false;
    }
    /**
     ...
     */
    generateFlutterStatePreservation(componentName, stateVars) {
...
${stateVars.map(v => `    ${v.name} = prefs.get${this.getPrefsMethod(v.type)}('${v.name}') ?? ${v.name};`).join('\n')}
     ...
     */
    generateReactStatePreservation(componentName, stateVars) {
        return `
...
${stateVars.map(v => `      ${v.name},`).join('\n')}
     ...
     */
    createStateSnapshot(state, definition) {
        return {
            timestamp: Date.now(),
            state: { ...state },
            definition: {
     ...
     */
    compareStateSnapshots(snapshot1, snapshot2) {
        const changes = [];

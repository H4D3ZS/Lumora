...
export type MessageType = 
  | 'connect'      // Initial connection from device
  | 'connected'    // Connection acknowledgment from server
  | 'update'       // Schema update from server to device
  | 'reload'       // Full reload request
  | 'error'        // Error notification
  | 'ping'         // Heartbeat from client
  | 'pong'         // Heartbeat response from server
  | 'ack';         // Acknowledgment of received update
 ...
 */
export function isConnectMessage(msg: HotReloadMessage): msg is ConnectMessage {
  return msg.type === 'connect';
}

export function isConnectedMessage(msg: HotReloadMessage): msg is ConnectedMessage {
  return msg.type === 'connected';
}

export function isUpdateMessage(msg: HotReloadMessage): msg is UpdateMessage {
  return msg.type === 'update';
}

export function isReloadMessage(msg: HotReloadMessage): msg is ReloadMessage {
  return msg.type === 'reload';
}

export function isErrorMessage(msg: HotReloadMessage): msg is ErrorMessage {
  return msg.type === 'error';
}

export function isPingMessage(msg: HotReloadMessage): msg is PingMessage {
  return msg.type === 'ping';
}

export function isPongMessage(msg: HotReloadMessage): msg is PongMessage {
  return msg.type === 'pong';
}

export function isAckMessage(msg: HotReloadMessage): msg is AckMessage {
  return msg.type === 'ack';
}

...
export class AnimationConverter {
  ...
  convertFramerMotionToFlutter(animation: AnimationDefinition): AnimationConversionResult {
    const result: AnimationConversionResult = {
    ...
    if (animation.type === 'spring' || animation.type === 'tween') {
      ...
      result.controllerDeclarations.push(
  ...
  private generateDraggableWidget(gesture: GestureDefinition): string {
    let code = `Draggable(\n`;
  ...
  private generateGestureDetector(gesture: GestureDefinition): string {
  ...
  private generateComplexGesture(gesture: GestureDefinition): string {
    let code = `GestureDetector(\n`;
  ...
  private getAnimationType(propertyName: string): string {
    if (propertyName === 'opacity') return 'double';
    if (propertyName === 'color') return 'Color';
    if (propertyName.includes('scale')) return 'double';
    if (propertyName.includes('rotate')) return 'double';
    if (propertyName === 'x' || propertyName === 'y') return 'double';
    if (propertyName.includes('translate')) return 'double';
    return 'double';
  }

  private convertValue(value: any, propertyName: string): string {
  ...
  private convertPropertyName(reactProp: string): string | null {
...
// Singleton instance
let animationConverterInstance: AnimationConverter | null = null;

export function getAnimationConverter(): AnimationConverter {

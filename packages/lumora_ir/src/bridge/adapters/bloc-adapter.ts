 ...
 */
export class BlocAdapter extends BaseStateAdapter {
  readonly name = 'bloc';
   ...
   * Generate state class
   */
  private generateStateClass(state: StateDefinition, stateName: string): string {
...
${copyWithBody}
   ...
   * Generate event classes
   */
  private generateEventClasses(state: StateDefinition, eventName: string): string {
    const baseEvent = `
abstract class ${eventName} extends Equatable {
   ...
   */
  private generateEventClass(variable: StateVariable, eventName: string): string {
...
${eventHandlers}
   ...
   */
  private generateEventHandler(variable: StateVariable, eventName: string, stateName: string): string {
    const eventClassName = `Update${this.capitalize(variable.name)}Event`;

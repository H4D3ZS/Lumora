...
export class DocSiteBuilder {
  private config: Required<DocSiteConfig>;
   ...
   */
  async buildSite(docs: APIDoc[]): Promise<void> {
...
lumora tutorial start getting-started</code></pre>
   ...
   */
  private async generateDocsPages(docs: APIDoc[]): Promise<void> {
    // Group docs by type
   ...
   */
  private async generateAPIIndex(grouped: Record<string, APIDoc[]>): Promise<void> {
    let content = '';
   ...
   */
  private async generateDocPage(doc: APIDoc, type: string): Promise<void> {
    let content = `
    ...
    await fs.writeFile(
      path.join(typeDir, `${doc.name}.html`),
      html
    );
  }
   ...
   */
  private async generateSearchIndex(docs: APIDoc[]): Promise<void> {
    ...
    await fs.writeFile(
      path.join(this.config.outputDir, 'js', 'search-index.js'),
      js
    );
  }
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener('click', function (e) {
    ...
    await fs.writeFile(
      path.join(this.config.outputDir, 'js', 'main.js'),
      mainJs
    );
...
if (searchInput) {
  ...
  document.addEventListener('click', (e) => {
    ...
    await fs.writeFile(
      path.join(this.config.outputDir, 'js', 'search.js'),
      searchJs
    );
  }
   ...
   */
  private wrapInTemplate(title: string, content: string, baseUrl: string = ''): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">

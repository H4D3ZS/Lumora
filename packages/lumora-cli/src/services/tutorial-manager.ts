...
export class TutorialManager {
  ...
  async startTutorial(tutorialId: string): Promise<void> {
    ...
    console.log(chalk.gray(`Duration: ~${tutorial.duration} minutes`));
    console.log(chalk.gray(`Steps: ${tutorial.steps.length}\n`));
   ...
   */
  private async displayStep(step: TutorialStep): Promise<void> {
    ...
    // Show code file
    if (step.codeFile) {
      console.log(chalk.cyan(`File: ${step.codeFile}`));
      console.log('');
    }

    // Show command
    if (step.command) {
      console.log(chalk.cyan('Run this command:'));
    ...
    if (step.checkCondition) {
      ...
      if (passed) {
        ...
        console.log(chalk.yellow('⚠ Complete the step before continuing\n'));
      }
    }
  }
  ...
  private displayCompletion(): void {
    if (!this.currentTutorial?.completion) return;

    console.log(chalk.bold.green('\n🎉 Tutorial Completed!\n'));
    console.log(chalk.white(this.currentTutorial.completion.message));
    console.log('');
   ...
   */
  private createFirstAppTutorial(): Tutorial {
    return {
      id: 'first-app',
...
<Button title="Step -1" onPress={() => setStep(Math.max(1, step - 1))} />`,
   ...
   */
  private createOTAUpdatesTutorial(): Tutorial {
    return {
      id: 'ota-updates',
   ...
   */
  private createNativeModulesTutorial(): Tutorial {
    return {
      id: 'native-modules',
...
Notifications.addListener('received', (notification) => {
   ...
   */
  private createStateManagementTutorial(): Tutorial {
    return {
      id: 'state-management',
...
function Counter() {
   ...
   */
  private createBuildingProductionTutorial(): Tutorial {
    return {

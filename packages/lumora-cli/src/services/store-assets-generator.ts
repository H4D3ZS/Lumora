...
export class StoreAssetsGenerator {
  // iOS App Icon Sizes (from Apple Human Interface Guidelines)
  private static readonly IOS_ICON_SIZES: AppIcon[] = [
    // iPhone
  ...
  // Screenshot Sizes
  private static readonly IOS_SCREENSHOT_SIZES: Screenshot[] = [
  ...
  private static readonly ANDROID_SCREENSHOT_SIZES: Screenshot[] = [
   ...
   */
  async generateAssets(config: StoreAssetsConfig): Promise<StoreAssetsResult> {
    const result: StoreAssetsResult = {
      icons: { ios: [], android: [] },
    ...
    if (config.screenshotSource) {
      result.screenshots.ios = await this.generateIOSScreenshots(
        config.screenshotSource,
        outputDir
      );
      result.screenshots.android = await this.generateAndroidScreenshots(
        config.screenshotSource,
        outputDir
      );
    }
   ...
   */
  private async generateIOSIcons(sourcePath: string, outputDir: string): Promise<string[]> {
   ...
   */
  private async generateAndroidIcons(sourcePath: string, outputDir: string): Promise<string[]> {
   ...
   */
  private async generateIOSScreenshots(
    sourcePath: string,
    outputDir: string
  ): Promise<string[]> {
   ...
   */
  private async generateAndroidScreenshots(
    sourcePath: string,
    outputDir: string
  ): Promise<string[]> {
   ...
   * Generate metadata templates
   */
  private async generateMetadataTemplates(
    appName: string,
    outputDir: string
  ): Promise<string[]> {
    ...
    await fs.writeFile(
      iosMetadata,
      this.generateIOSMetadataTemplate(appName)
    );
    ...
    await fs.writeFile(
      androidMetadata,
      this.generateAndroidMetadataTemplate(appName)
    );
   ...
   */
  private async generateIOSContentsJson(outputDir: string): Promise<void> {
    const contentsJson = {
   ...
   */
  private generateIOSMetadataTemplate(appName: string): string {
...
`;
   ...
   */
  private generateAndroidMetadataTemplate(appName: string): string {
...
`;
   ...
   */
  private generatePrivacyPolicyTemplate(appName: string): string {
...
// Singleton instance
let generatorInstance: StoreAssetsGenerator | null = null;

export function getStoreAssetsGenerator(): StoreAssetsGenerator {

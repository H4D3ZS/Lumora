...
import {
  createGitHubClient,
  createGitHubGistManager,
  createGitHubGistLoader,
} from '../index';
...
async function exampleUpdateGist(gistId: string) {
  const client = createGitHubClient({
    auth: {
      type: 'pat',
      token: process.env.GITHUB_TOKEN || 'your-token-here',
...
async function exampleForkGist(gistId: string) {
  const client = createGitHubClient({
    auth: {
      type: 'pat',
      token: process.env.GITHUB_TOKEN || 'your-token-here',
...
async function exampleLoadGist(gistId: string) {
  const client = createGitHubClient({
    auth: {
      type: 'pat',
      token: process.env.GITHUB_TOKEN || 'your-token-here',
...
async function exampleUpdateGistProject(projectPath: string) {
  const client = createGitHubClient({
    auth: {
      type: 'pat',
      token: process.env.GITHUB_TOKEN || 'your-token-here',
  ...
  try {
    switch (command) {
      case 'create':
        ...
        break;
      case 'update':
        if (!arg) throw new Error('Gist ID required');
      ...
      case 'list':
        ...
        break;
      case 'update-project':
        if (!arg) throw new Error('Project path required');

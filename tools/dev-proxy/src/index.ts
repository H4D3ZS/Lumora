...
import { WebSocketBroker } from './websocket-broker';
import { ErrorResponse, WebSocketEnvelope } from './types';
 ...
 */
app.post('/send/:sessionId', (req: Request, res: Response) => {
  const { sessionId } = req.params;
  ...
  try {
    const envelope: WebSocketEnvelope = req.body;
...
const wsBroker = new WebSocketBroker(server, sessionManager);
...
// Export for testing
export { app, sessionManager, wsBroker };
